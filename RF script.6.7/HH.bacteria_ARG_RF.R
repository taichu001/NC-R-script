...
set.seed(123)
factor_rfP <- rfPermute(HH.ARG ~ ., data = data_pca, importance = TRUE, ntree = 500, nrep = 1000, num.cores = 6)
factor_rfP
...
set.seed(123)
factor_forest.pval <- a3(HH.ARG ~ ., data = data_pca, model.fn = randomForest, p.acc = 0.001, model.args = list(importance = TRUE, ntree = 500))
factor_forest.pval
...
ggsave(p, file = "HH.ARG_rf.pdf", width = 5, height = 7)
